...
from deebot_client.commands.life_span import (
    GetLifeSpanBrush,
    GetLifeSpanHeap,
...
@pytest.mark.parametrize(
    ...
    [
        (
            get_request_xml(
                "<ctl ret='ok' type='SideBrush' left='8977' total='9000'/>"
            ),
            LifeSpanEvent(LifeSpan.SIDE_BRUSH, 99.74, 8977),
        ),
    ],
)
async def test_GetLifeSpanSideBrush(
    json: dict[str, Any], expected: LifeSpanEvent
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (
            get_request_xml(
                "<ctl ret='ok' type='DustCaseHeap' left='7179' total='7200'/>"
            ),
            LifeSpanEvent(LifeSpan.FILTER, 99.71, 7179),
        ),
    ],
)
async def test_GetLifeSpanDustCaseHeap(
    json: dict[str, Any], expected: LifeSpanEvent
) -> None:
