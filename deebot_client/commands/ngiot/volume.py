...
from .common import APN_SET_VOLUME, NgiotExecuteCommand, RobotDetailGetCommand
...
class SetVolume(NgiotExecuteCommand):
    ...
    async def _request_ngiot(
        ...
        if not self.MIN_VOLUME <= self._volume <= self.MAX_VOLUME:
            msg = f"Volume must be between {self.MIN_VOLUME} and {self.MAX_VOLUME}"
    ...
    def _handle_response(
        ...
        if result.state == HandlingState.SUCCESS:
            event_bus.notify(VolumeEvent(volume=self._volume, maximum=self.MAX_VOLUME))
        return result
