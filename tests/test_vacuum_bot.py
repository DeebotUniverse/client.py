...
async def test_available_check_and_teardown(
    ...
    received_statuses: asyncio.Queue[AvailableEvent] = asyncio.Queue()

    async def on_status(event: AvailableEvent) -> None:
    ...
    with patch("deebot_client.vacuum_bot.GetBattery", spec_set=True) as battery_command:
        ...
        bot.events.subscribe(AvailableEvent, on_status)
