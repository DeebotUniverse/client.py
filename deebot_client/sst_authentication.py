...
class SstAuthenticator:
    ...
    async def _issue_sst(self, identity: SstDeviceIdentity) -> SstCredentials:
        ...
        try:
            ...
            _LOGGER.debug(
                "SST issue response for %s: %s", identity.key, response_data
            )
            ...
            token = str(response_data["data"]["data"]["token"])
    ...
    def _create_refresh_task(
        ...
        self._refresh_handles[identity.key] = asyncio.get_event_loop().call_later(
    ...
    def _normalize_device(
        ...
        control_host = (
            service.get("mqs") if isinstance(service, Mapping) else None
        )
        ...
        return SstDeviceIdentity(
            ...
            control_host=str(control_host) if control_host else None,
        )
