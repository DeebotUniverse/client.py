...
const WALL_STROKE_COLOR: &str = "#4e96e2";
const WALL_STROKE_WIDTH: f32 = 3.0;
const FLOOR_FILL_COLOR: &str = "#badaff";
const POSSIBLE_OBSTACLE_FILL_COLOR: &str = "#edf3fb";
...
impl MapInfo {
    ...
    pub(super) fn generate(&self) -> Option<(Vec<Box<dyn svg::node::Node>>, ViewBox)> {
        ...
        for polygon in add_polygons_to_svg(&self.areas, POSSIBLE_OBSTACLE_FILL_COLOR) {
            svg_elements.push(Box::new(polygon));
        }
        for polygon in add_polygons_to_svg(&self.block_lines, FLOOR_FILL_COLOR) {
            svg_elements.push(Box::new(polygon));
        }
        ...
        for outline in &self.outlines {
            if let Some(d) = points_to_svg_path(outline) {
                svg_elements.push(Box::new(
                    Path::new()
                        .set("fill", "none")
                        .set("stroke", WALL_STROKE_COLOR)
                        .set("stroke-width", WALL_STROKE_WIDTH)
    ...
    fn parse_map_info(&mut self, info: MapV2Info) {
        for group in &info {
            ...
            match first.as_str() {
                ...
                MAP_INFO_TYPE_AREA => self.areas = process_map_info_polygon_entries(group),
